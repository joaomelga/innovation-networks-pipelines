"""@bruin
name: experiment.wnodf_temporal
image: python:3.13
...
description: |
  Almeida-Neto & Ulrich (2011) weighted NODF (WNODF) computed temporally,
  per clustering method and community. Edge weight = count of distinct
  portfolio companies per (early, late) pair within the time window.
...
from lib.nestedness.nodf import compute_wnodf
...
def _trim_and_compute(rows: np.ndarray, cols: np.ndarray, shape: tuple) -> dict:
    ...
    if n_left < 2 or n_right < 2:
        return {**base, "fill": np.nan, "wnodf": np.nan, "wnodf_rows": np.nan, "wnodf_cols": np.nan}
    return {**base, **compute_wnodf(W)}
