"""@bruin
name: experiment.johnson_nestedness_weighted_temporal
image: python:3.13
...
description: |
  Temporal weighted Johnson et al. (2013) nestedness per community and
  clustering method. Edge weight = count of distinct portfolio companies
  per (early, late) pair within the time window.
...
def compute_johnson(biadj: sp.csr_matrix) -> dict:
    ...
    sum_top, sum_bot = float(G_top.sum()), float(G_bot.sum())
    N = n_rows + n_cols
    num_pairs = N * (N - 1)
    if num_pairs <= 0:
        return nan_out
    ...
    n1, n2 = n_rows, n_cols
    k_mean_l, k_mean_r = k_left.mean(), k_right.mean()
    ...
    if n1 > 1 and n2 > 1 and denom > 0:
        ...
        g_norm_left = sum_top / (n_rows * n_rows) / g_conf_left if g_conf_left > 0 else np.nan
        g_norm_right = sum_bot / (n_cols * n_cols) / g_conf_right if g_conf_right > 0 else np.nan
    else:
        g_norm_left = g_norm_right = np.nan
    return {
        "g_raw": float(g_raw), "g_conf": float(g_conf), "g_norm": float(g_norm),
        "g_norm_left": float(g_norm_left), "g_norm_right": float(g_norm_right),
    }
ROLLING_WINDOW = 5
MAX_WORKERS = min(8, (os.cpu_count() or 2))


def _build_weighted_biadj(rows: np.ndarray, cols: np.ndarray, shape: tuple) -> sp.csr_matrix:
    """Weighted biadjacency: duplicate COO entries are summed, yielding
    COUNT(DISTINCT company) per (Source, Target) pair."""
    if rows.size == 0:
...
def _trim_and_compute(rows: np.ndarray, cols: np.ndarray, shape: tuple) -> dict:
    W = _build_weighted_biadj(rows, cols, shape)
...
def materialize() -> pd.DataFrame:
    ...
    print(f"\nWeighted Johnson nestedness temporal: {len(df)} rows")
    return df
